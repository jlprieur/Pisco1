...
compact_string(compacted_object_name, 40);
...
*pc = '\0';
compact_string(object_in_catalog, 40);
...
compact_string(compacted_object, 64);
pc = compacted_object;
object_len = 0;
while(*pc) {
...
*/
    ...
    compact_string(compacted_line,128);
...
/* Look for the discoverer's name the 2nd line */
         ...
         compact_string(compacted_discov, 64);
         break;
...
*********************************************************************/
int read_coordinates_from_PISCO_catalog(char *in_line, float *alpha, 
...
nval = sscanf(buffer,"%d %d %f", &i1, &i2, &d3);
if(nval != 3) {
  fprintf(stderr, "PISCO_catalog_read_line1/Error: bad right ascension!\n");
  return(-1);
  }
*alpha_Pcat = (float)i1 + ((float)i2)/60. + d3/3600.;
...
if(i1 < 0) i1 *= -1;
*delta_Pcat = sign * ((float)i1 + ((float)i2)/60. + ((float)i3)/3600.);
 ...
 *pc0 = '\0';
 if((nval = sscanf(buffer0, "%f", &val0)) == 1) *magV_A = val0;
...
/* Save following characters to buffer0: */
   ...
   while(*pc0 != ' ' && *pc0) pc0++;
   *pc0 = '\0';
   if((nval = sscanf(buffer0, "%f", &val0)) == 1) *magV_B = val0;
...
/* Cut the string when the first " " or alpha character is found: */
 while(*pc0 !=' ' && !isalpha(*pc0) && *pc0) pc0++;
 *pc0 = '\0';
 if((nval = sscanf(buffer0, "%f", &val0)) == 1) *magV = val0;
}
...
/* Cut the string when the first " " or alpha character is found: */
 while(*pc0 !=' ' && !isalpha(*pc0) && *pc0) pc0++;
 *pc0 = '\0';
 if((nval = sscanf(buffer0, "%f", &val0)) == 1) *B_V = val0;
}
...
/* Save the error to buffer1 if '('present: */
 if(*pc0 == '(') { 
   ...
   *pc1 = '\0';
   if((nval = sscanf(buffer1, "%f", &val0)) == 1) *err_paral = val0;
   }
/* Decode parallax: */
 *pc0 = '\0';
 if((nval = sscanf(buffer0, "%f", &val0)) == 1) *paral = val0;
}
