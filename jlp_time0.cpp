...
* int jlp_italian_date(char *cdate, char *ctime);
* int jlp_french_date(char *cdate, char *ctime);
*
*  JLP
* Version 05-08-2015
**********************************************************************/
...
int JLP_TIME_MSEC(char *string, int *istat)
{
 ...
 struct timeval tt;
  ...
  sprintf(string, "%s [%ld:%06ld]\n", str, tt.tv_sec, tt.tv_usec);
  return(0);
}
/*****************************************************************
...
// LINUX
  ...
  gettimeofday(&tt, NULL);
  JLP_CTIME(str, istat);
  sprintf(full_date, "%s [%ld:%06ld]\n", str, tt.tv_sec, tt.tv_usec);
// WIN32 API
#else
  SYSTEMTIME st;
  GetSystemTime(&st);
  jlp_local_time(str);
  sprintf(full_date, "%s [%02d.%03d]\n", str, st.wSecond, st.wMilliseconds);
#endif
...
* Example of output:
*     "Wed Jun 19 12:05:34 1994"
*     cdate="Giovedi 19 Giugno 1994"  ctime="12:05:04"
********************************************************************/
int jlp_italian_date(char *cdate, char *ctime)
{
