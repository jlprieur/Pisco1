...
char comports[38][16]={
    ...
    "/dev/ircomm0","/dev/ircomm1","/dev/cuau0","/dev/cuau1","/dev/cuau2",
    "/dev/cuau3","/dev/cuaU0","/dev/cuaU1","/dev/cuaU2","/dev/cuaU3"
    };
...
*
***************************************************************************/
int RS232_PollComport(int comport_number, char *buf, int size, int *nread)
{
  int n;
...
/***************************************************************
 * Name: RS232_Find_Ports
 * JLP version 2022 
 **************************************************************/
int RS232_Find_Ports(int* iports, int nports_maxi)
{
printf("RS232_Find_Ports: not yet available for LINUX \n");
return(1);
}
...
*/
// Semi-blocking mode (from cpp version)
...
// JLP:
  Cptimeouts.WriteTotalTimeoutConstant   = 0;
/* JLP2015:
// Semi-blocking mode (wait until there is a gap of 1 ms in the data)
  Cptimeouts.ReadIntervalTimeout       = 1;
  Cptimeouts.ReadTotalTimeoutMultiplier  = 0;
// Wait a max of 1 ms before getting a first byte:
...
* Version: JLP 2015
* Return status=0 if OK, or something else if port is waiting
****************************************************************/
int RS232_PollComport(int comport_number, char *buf, int size, int *nread)
{
int status;
...
//  status = ReadFile(Cport[comport_number], buf, size, (LPDWORD)((void *)&n), NULL);
if(ReadFile(Cport[comport_number], buf, size, &n, NULL)) {
  status = 0;
  *nread = (int)n;
} else {
  status = -1;
...
/***************************************************************
* Purge serial port (new routine: JLP 2015)
****************************************************************/
void RS232_PurgeComport(int comport_number)
{
// Clears the input buffer (if the device has one):
...
/***************************************************************
 * Name: RS232_Find_Ports
 * From Andres Cassagnes (2014)
 * JLP version 28/10/2015
 **************************************************************/
int RS232_Find_Ports(int* iports, int nports_maxi)
{
HANDLE DriverHandle;
...
int i, nports = 0;
    ...
    for(i = 0 ; i < nports_maxi ; i++)
    {
      sprintf(PortName, "COM%d", i+1);
      DriverHandle = CreateFile (PortName, 0, 0, NULL, OPEN_EXISTING, 0, NULL);
      LastError[i] = GetLastError();
      if(LastError[i] == 0) {
        iports[nports] = i + 1;
        nports++;
        }
