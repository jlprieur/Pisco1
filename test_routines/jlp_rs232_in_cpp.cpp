...
int MyRS232::OpenComport(int comport_number)
{
...
int MyRS232::PollComport(int comport_number, char *buf, int size)
{
  DWORD n;
  int b;
...
int MyRS232::SendBuf(int comport_number, char *buf, int size)
{
  int n;
...
void MyRS232::CloseComport(int comport_number)
{
  CloseHandle(Cport[comport_number]);
}

void MyRS232::Reset(int comport_number) //Tested 24/02/12
{
...
void MyRS232::flush(int comport_number)
{
    int n=1;
    char buf[255];
    while(n>0)
